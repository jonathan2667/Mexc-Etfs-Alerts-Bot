...
def write_symb(cnt):
    with open(r"C:\Users\jmogo\PycharmProjects\mexc1\test4.txt", 'r') as f:
    ...
    print(cnt)
    cnt = 0
    with open('symbols3xETFS.txt', 'w') as f:
        for line in symb:
            cnt += 1
            if (cnt % 2 == 0) :
                f.write(line)
...
def write_cont() :
    with open(r"ContFile.txt", 'r') as f:
        datafile = f.readlines()
...
def get_ts_rebalancing() :
    now = datetime.now()  # important
    time_rebalance = datetime(now.year, now.month, now.day, 16, 0, 0)
    if (now.hour < 16):
        time_rebalance = datetime(now.year, now.month, now.day, 16, 0, 0) - timedelta(days = 1)
...
def get_rebalancing_price(symbol_name) :
    r = requests.get("https://contract.mexc.com/api/v1/contract/kline/" + symbol_name + "_USDT?interval=Min1&start=" + str(get_ts_rebalancing()) + "&end=" + str(get_ts_rebalancing()))
...
while(1):
    now = datetime.now()
    if (now.hour != hour):
        #bot.send_message(chat_id,  str(now.strftime("%Y-%m-%d %H:%M:%S")))
        hour = now.hour
  
    with open(r"symbols3xETFS.txt", 'r') as f:
        datafile = f.readlines()
    ...
    for line in datafile:
        try :
            ...
            percentage_increase = round(((curr_price - rebal_price) * 1.00 / rebal_price) * 100.00, 2)
            percentage_decrease = round(((rebal_price - curr_price) * 1.00 / rebal_price) * 100.00, 2)
                ...
                if (percentage_increase > 10):
                    ...
                    if (high_price < curr_price and curr_price < 1.18 * rebal_price):
                        ...
                        text2 = line + " 3X CLOSE TO REBALANCING UP: \n" + \
                                "Percentage: " + str(percentage_increase) + \
                                "% \nLast Rebalance Price: " + str(rebal_price) + \
        ...
        except (KeyError, IndexError, urllib3.exceptions.MaxRetryError, SSLError) as e:
            print(line + " " + str(e))
